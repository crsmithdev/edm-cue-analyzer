...
def analyze_single_track(audio_file: str, output_xml: str = None):
    ...
    structure = analyzer.analyze_file(Path(audio_file))
...
def batch_analyze_tracks(audio_directory: str, output_directory: str):
    """Analyze multiple tracks in a directory."""
...
def custom_cue_workflow(audio_file: str):
    """Example of customizing cue generation programmatically."""
    
    from edm_cue_analyzer.config import Config, CueConfig, AnalysisConfig
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == '--batch':
        if len(sys.argv) < 4:
            ...
            sys.exit(1)
        batch_analyze_tracks(sys.argv[2], sys.argv[3])
    else:
        audio_file = sys.argv[1]
        output_xml = sys.argv[2] if len(sys.argv) > 2 else None
        analyze_single_track(audio_file, output_xml)
